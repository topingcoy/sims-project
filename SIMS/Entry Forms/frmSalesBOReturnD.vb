...
Public Class frmSalesBOReturnD
    ...
    Sub GetTransNo()
        ...
        If mdr.HasRows = True Then
            ...
            rcvtno = "B" & Strings.StrDup(9 - Strings.Len(rcvtno), "0") & rcvtno
            mdr.Close()
        Else
            mdr.Close()
            mdr.Dispose()
    ...
    Sub LoadItemList()
        ...
        cmd = New MySqlCommand("SELECT * FROM ivitem WHERE obsolet <> 'Y' and isdeleted<>1 ORDER BY supplier ASC", cdb)
    ...
    '    Dim cmd As MySqlCommand
    '    'cmd = New MySqlCommand("SELECT PCKGC,PCKGB,PCKGP,GRSCOST,UNIT, NETCOST, COSTPC FROM ivitem WHERE ITEMCODE=@tm", cdb)
    '    cmd = New MySqlCommand("SELECT * FROM ivitem WHERE ITEMCODE=@tm", cdb)
    '    cmd.Parameters.AddWithValue("@tm", itm)
    '    Using rd As MySqlDataReader = cmd.ExecuteReader
    '        If rd.HasRows Then
    '            rd.Read()
    ...
    Private Sub dgvItemList_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs)
        load_items()
    End Sub
    ...
    Private Sub txtCustCode_DoubleClick(sender As Object, e As EventArgs) Handles txtCustCode.DoubleClick
        frmSearchCustomerInvoice.Text = "BO RETURN"
    ...
    Private Sub txtCustCode_MouseDoubleClick(sender As Object, e As MouseEventArgs) Handles txtCustCode.MouseDoubleClick
    ...
    Private Sub frmSalesBOReturnD_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmSalesBOReturnD_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub txtSalesmanCode_DoubleClick(sender As Object, e As EventArgs) Handles txtSalesmanCode.DoubleClick
        frmSearchSalesmanBO.ShowDialog()
    ...
    Function checkInvoiceNo()
        ...
        If txtInvoiceNo.Text.Trim <> Nothing Then
            Dim cmd As MySqlCommand
            cmd = New MySqlCommand("SELECT invoiceno FROM salesh WHERE invoiceno = @ivno", cdb)
            With cmd
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If txtInvoiceNo.Text = "" Then
            MsgBox("BO Return slip no is required", vbCritical)
            txtInvoiceNo.Focus()
            Exit Sub
        End If

        If checkInvoiceNo() = True Then
            MsgBox("BO Return slip no. already used!.", 48, "Enter slip no.")
            txtInvoiceNo.Focus()
            Exit Sub
        End If
        ...
        With dgvItem
            ...
            If .RowCount > 0 Then
                ...
                If asave = True Then
                    ' GetTransNo()
                    SaveBOReturn()
                    MsgBox("BO return has been save successfully", vbInformation)
                    iClear()
                    txtCustCode.Focus()
                    With frmSalesBOReturnH
                        '.LoadReturnHeader()
                        .LoadBOReturnHeader()
                    End With
                Else
    ...
    Sub SaveBOReturn()
        'insert to salesh
        Dim TransNo As String = GenerateTransNo_SalesInvoice_Return_Bo("B")
        Dim cmd As MySqlCommand
        Dim i As Integer
        cmd = New MySqlCommand("INSERT INTO salesh (transno,ttype,tdate,custcode,slmncode,whscode,invoiceno,pono,grosstot,disc1percent,disc1amount,disc2percent,disc2amount, " & _
                               "remarks,totalamt,user,terms,disc1net,disc2net) VALUES (@transno,@ttype,@tdate,@custcode,@slmncode,@whscode,@invoiceno,@pono,@grosstot,@disc1percent,@disc1amount, " & _
                               "@disc2percent,@disc2amount,@remarks,@totalamt,@user,@terms,@disc1net,@disc2net)", cdb)
        With cmd
                ...
                If .Item(3, i).Value <> Nothing Then
                    If .Item(3, i).Value > 0 Then
                        cmd = New MySqlCommand("INSERT INTO salesd (transno,ttype,tdate,pricetype,whscode,invoiceno, " & _
                                               ...
                                               "@whscode,@invoiceno,@pono,@itemcode,@qty,@qunit,@pckg,@uprice,@amount)", cdb)
                        cmd.Parameters.AddWithValue("@transno", TransNo)
                        cmd.Parameters.AddWithValue("@ttype", ttype)
                        cmd.Parameters.AddWithValue("@tdate", dtpInvoiceDate.Value)
                        ...
                        cmd.Parameters.AddWithValue("@invoiceno", txtInvoiceNo.Text)
                        cmd.Parameters.AddWithValue("@pono", txtPONo.Text)
                        cmd.Parameters.AddWithValue("@itemcode", .Item(1, i).Value)
            ...
            For i = 0 To .RowCount - 1
                If .Item(3, i).Value <> Nothing Then
                    If .Item(3, i).Value > 0 Then
                        ...
                        Dim current_quantity As String = Nothing

                        qunit = .Item(4, i).Value
                        current_quantity = .Item(3, i).Value

                        Dim fnds As Boolean = False
                        Dim curqty As Decimal = 0
                        cmd = New MySqlCommand("SELECT itemcode,qty_bo FROM ivwh WHERE itemcode=@itm AND whscode=@wh", cdb)
                        ...
                        itmc = .Item(1, i).Value
                        totalqty = convertpackaging_fromivitem(itmc, qunit, current_quantity, qunit)

                        If fnds = False Then
                            cmd = New MySqlCommand("INSERT INTO ivwh (itemcode,whscode,qty_bo) VALUES (@itm,@wh,@qtyg)", cdb)
                        ...
                        Else
                            ...
                            nqty = curqty + totalqty
                            cmd = New MySqlCommand("UPDATE ivwh SET qty_bo=@qtyg WHERE itemcode=@itm AND whscode=@wh", cdb)
                            cmd.Parameters.AddWithValue("@itm", .Item(1, i).Value)
                            cmd.Parameters.AddWithValue("@wh", txtWarehouseCode.Text.Trim)
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        ...
        If txtInvoiceNo.Text <> txtInvoiceUpdate.Text Then
            If checkInvoiceNo() = True Then
                MsgBox("Return slip no. already used!.", 48, "Enter return slip no.")
                txtInvoiceNo.Focus()
                Exit Sub
            End If
        End If
  ...
  'NEW PROCESS UPDATE IVH => COCOY
        ...
        cmd = New MySqlCommand("UPDATE salesh SET ttype=@ttype,tdate=@tdate,custcode=@custcode, " & _
                               ...
                               "remarks=@remarks,totalamt=@totalamt,user=@user,terms=@terms,disc1net=@disc1net,disc2net=@disc2net WHERE transno=@transno", cdb)
        With cmd
        ...
        With dgvItem
            ...
            For x As Integer = 0 To rwrmv - 1
                ...
                ' delstat = 1
                'cmd = New MySqlCommand("UPDATE salesd SET isdeleted='1' WHERE transno=@tno AND itemcode=@itemc", cdb) 'original
                cmd = New MySqlCommand("UPDATE salesd SET isdeleted='1' WHERE transno=@tno AND itemcode=@itemc AND pkno=@pno", cdb)
                ...
                curqty = getcurqty(rmvitc, whsecode, "B")
            ...
            For i = 0 To .RowCount - 1
                If .Item(3, i).Value <> Nothing Then 'qty <> nothing
                    If .Item(3, i).Value > 0 Then 'qty > 0
                        ...
                        qtyfromwhs = getcurqty(itmc, whsecode, "B")
                        ...
                        If old_item_unit <> Nothing Then
                            If old_item_unit <> unt Then
                                ...
                                return_diff_to_whs_bo(itmc, old_item_qty, whsecode, "+")
                                'End If
                                oqty = new_item_qty
                        ...
                        If fnd1 = True Then
                            ...
                            cmd = New MySqlCommand("UPDATE salesd SET ttype=@ttype,tdate=@tdate, " & _
                                                   ...
                                                   "invoiceno=@invoiceno,pono=@pono,itemcode=@itemcode,qty=@qty,qunit=@unit,pckg=@pckg, " & _
                            ...
                            If markunitchange = 1 Then
                                update_currentwhs_1and2_bo(itmc, oqty, nwq, prevwhs, whsecode, whsecode2, my_operator)
                            Else
                                old_item_qty = convertpackaging_fromivitem(itmc, pkn, oqty, old_item_unit)
                                update_currentwhs22_bo(itmc, old_item_qty, nwq, whsecode2, whsecode, pvwhs, my_operator)
                            End If
                        Else
                            cmd = New MySqlCommand("INSERT INTO salesd (transno,ttype,tdate,pricetype,whscode,invoiceno, " & _
                                             "pono,itemcode,qty,qunit,pckg,uprice,amount) VALUES (@transno,@ttype,@tdate,@pricetype, " & _
                                             "@whscode,@invoiceno,@pono,@itemcode,@qty,@unit,@pckg,@uprice,@amount)", cdb)
                            cmd.Parameters.AddWithValue("@transno", txtTransNo.Text)
                            cmd.Parameters.AddWithValue("@ttype", ttype)
                            cmd.Parameters.AddWithValue("@tdate", dtpInvoiceDate.Value)
                            cmd.Parameters.AddWithValue("@pricetype", txtPriceType.Text)
                            ...
                            nwq = convertpackaging_fromivitem(itmc, pkn, newwqty, unt)
                            old_item_qty = getcurqty(itmc, whsecode, "B")
                            update_currentwhs_stockin_bo(itmc, old_item_qty, nwq, whsecode2, whsecode, nwq, my_operator)
                        End If
                    End If
                End If
            Next
        ...
        With dgvItem
            ...
            If .RowCount > 0 Then
                ...
                If asave = True Then
                    GetTransNo()
                    SaveBOReturn()
                    MsgBox("BO return has save successfully", vbInformation)
                    PrintSalesInvoice()
                    iClear()
                    With frmSalesBOReturnH
                        .LoadBOReturnHeader()
            ...
            If txtWarehouseCode.Text <> Nothing Then
                If MsgBox("Update changes?", vbYesNo + vbQuestion) = vbYes Then
                    ...
                    If asave = True Then
                        .EndEdit(True)
                        UpdateBOReturn()
                        MsgBox("BO return has been updated successfully.", vbInformation)
                        PrintSalesInvoice()
                        iClear()
                        With frmReport
                            .BringToFront()
    ...
    Private Sub dgvRcvItem_EditingControlShowing(sender As Object, e As DataGridViewEditingControlShowingEventArgs) Handles dgvItem.EditingControlShowing
        With dgvItem
            If .Rows.Count > 0 Then
                If .CurrentCell.ColumnIndex = 3 Then
                    RemoveHandler CType(e.Control, TextBox).KeyPress, AddressOf column_keypress
    ...
    Sub RetrieveBOReturn(ByVal trno As String)
        ...
        With dgvItem
            ...
            cmd = New MySqlCommand("SELECT h.*,w.*,s.*,c.* FROM salesh h LEFT JOIN warehouse w ON h.whscode=w.whscode " & _
                                   ...
                                   "WHERE h.isdeleted<>1 AND ttype='B' AND h.transno=@tno", cdb)
            ...
            cmd = New MySqlCommand("SELECT d.*,i.* FROM salesd d LEFT JOIN ivitem i ON d.itemcode=i.itemcode " & _
                                   "WHERE d.isdeleted<>1 and d.ttype='B' and d.transno=@tno ORDER BY d.pkno", cdb)
            cmd.Parameters.AddWithValue("@tno", trno)
            Using rd As MySqlDataReader = cmd.ExecuteReader
                If rd.HasRows Then
                    x = 0
                    ...
                    While rd.Read
                        ...
                        .Item(4, rw).Value = rd("unit").ToString
                        .Item(4, rw).Tag = rd("unit").ToString
                        .Item(5, rw).Value = rd("PCKG").ToString

                        If rd("unit").ToString = "C" Or rd("unit").ToString = "B" Then
                            totalqty = rd("QTY").ToString * IIf(rd("PCKG").ToString > 0, rd("PCKG").ToString, 1)
                        ElseIf rd("unit").ToString = "P" Then
    ...
    Private Sub btnCloseD_Click(sender As Object, e As EventArgs) Handles btnCloseD.Click
        If MsgBox("Are you sure you want to discard changes?", vbYesNo + vbQuestion, "Sales Invoice") = vbYes Then
            ...
            close_open_trans_by_user(trno, "salesh", _user)
            Me.Dispose()
    ...
    'Private Sub btnSelectSalesman_Click(sender As Object, e As EventArgs) Handles btnSelectSalesman.Click
    ...
    Private Sub getpackprice(itemno As String, mpc As String, mrw As Integer)
        Throw New NotImplementedException
    End Sub

    Private Sub dgvItemList_CellContentClick(sender As Object, e As DataGridViewCellEventArgs)
    ...
    Private Sub dgvItemList_CellContentClick_1(sender As Object, e As DataGridViewCellEventArgs) Handles dgvItemList.CellContentClick
    ...
    Private Sub dgvItemList_CellDoubleClick1(sender As Object, e As DataGridViewCellEventArgs) Handles dgvItemList.CellDoubleClick
        load_items()
    End Sub
End Class