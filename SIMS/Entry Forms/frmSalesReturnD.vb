...
Public Class frmSalesReturnD
        ...
        With dgvItem
            Using rd As MySqlDataReader = cmd.ExecuteReader
                If rd.HasRows Then
                    ...
                    .Item(7, rdx).Value = 0 'rd("netcost").ToString
    ...
    Private Sub tbSearchItem_KeyDown(sender As Object, e As KeyEventArgs) Handles txtSearchItem.KeyDown
        If e.KeyCode = Keys.Down Then
            dgvItemList.Focus()
        End If
    End Sub

    Private Sub tbSearchItem_TextChanged(sender As Object, e As EventArgs) Handles txtSearchItem.TextChanged
        Try
            Dim i As Integer
            dgvItemList.Rows.Clear()
            cmd = New MySqlCommand("SELECT * from ivitem where (itemcode like '%" & txtSearchItem.Text & "%' " & _
                                   " OR description like '%" & txtSearchItem.Text & "%') and isdeleted<>1 ", cdb)
    ...
    Private Sub txtCustCode_DoubleClick(sender As Object, e As EventArgs) Handles txtCustCode.DoubleClick
        frmSearchCustomerInvoice.Text = "SALES RETURN"
    ...
    Private Sub frmSalesReturnD_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmSalesReturnD_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub txtSalesmanCode_DoubleClick(sender As Object, e As EventArgs) Handles txtSalesmanCode.DoubleClick
        frmSearchSalesmanRet.ShowDialog()
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If txtInvoiceNo.Text = "" Then
            MsgBox("Return slip no is required", vbCritical)
            txtInvoiceNo.Focus()
            Exit Sub
        End If

        If checkInvoiceNo() = True Then
            MsgBox("Return slip no. already used!.", 48, "Enter slip no.")
            txtInvoiceNo.Focus()
            Exit Sub
        End If
        ...
        With dgvItem
            ...
            If .RowCount > 0 Then
                ...
                If asave = True Then
                    ' GetTransNo()
                    SaveSalesReturn()
                    MsgBox("Sales return has been save successfully", vbInformation)
                    iClear()
                    txtCustCode.Focus()
                    With frmSalesReturnH
                        '.LoadReturnHeader()
                        .LoadSalesReturnHeaderNew()
                    End With
                Else
    ...
    Sub SaveSalesReturn()
        ...
        Dim TransNo As String = GenerateTransNo_SalesInvoice_Return_Bo("R")
        Dim cmd As MySqlCommand
        Dim i As Integer
        ...
        '                itmc = .Item(1, i).Value
        '                'totalqty = convertpackaging_fromivitem(itmc, qunit, current_quantity, qunit) ' old
        ...
        'insert to ivwh (copied from stock in)
        With dgvItem
            For i = 0 To .RowCount - 1
                If .Item(3, i).Value <> Nothing Then
                    If .Item(3, i).Value > 0 Then
            ...
            If txtWarehouseCode.Text <> Nothing Then
                If MsgBox("Update changes?", vbYesNo + vbQuestion) = vbYes Then
                    ...
                    If asave = True Then
                        .EndEdit(True)
                        UpdateSalesReturn()
                        ' MsgBox("Return has been updated successfully.", vbInformation)
                        iClear()
                        Me.Dispose()
        ...
        With dgvItem 'sa pagdugang ni
            ...
            If mkerr = 0 Then
                ...
                For x As Integer = 0 To rwrmv - 1
                    ...
                    cmd.Parameters.AddWithValue("@qtyg", rmvwhse)
                    cmd.ExecuteNonQuery()
                    cmd.Dispose()
                ...
                For i = 0 To .RowCount - 1
                    If .Item(3, i).Value <> Nothing Then 'qty <> nothing
                        If .Item(3, i).Value > 0 Then 'qty > 0
                            ...
                            cmd = New MySqlCommand("SELECT itemcode FROM salesd WHERE pkno=@pno and transno=@tno and itemcode=@itm and isdeleted='0'", cdb)
        ...
        With dgvItem
            ...
            If .RowCount > 0 Then
                ...
                For i = 0 To .RowCount - 1
                    ...
                    r1("rtitle") = "SALES RETURN"
                    r1("dtitle") = "Return Slip No.:"
                    ...
                    'r1("customername") = txtCustomername.Text.ToString
                    'r1("address") = txtAddress.Text.ToString
                    'r1("slmncode") = txtSalesmanCode.Text.ToString
                    'r1("slmnname") = txtSalesmanName.Text.ToString
                    'r1("pricetype") = txtPriceType.Text.ToString
                    'r1("whscode") = txtWarehouseCode.Text.ToString
                    'r1("whsname") = txtWarehouseName.Text.ToString
                    'r1("pono") = txtPONo.Text.ToString
                    'r1("invoiceno") = txtInvoiceNo.Text.ToString
                    'r1("itemcode") = IIf(.Item(1, i).Value = Nothing, DBNull.Value, .Item(1, i).Value)
                    'r1("description") = IIf(.Item(2, i).Value = Nothing, DBNull.Value, .Item(2, i).Value)
                    ...
                    'r1("totqtycase") = lblCase.Text.ToString
                    'r1("totqtybag") = lblBag.Text.ToString
                    'r1("totqtypc") = lblPcs.Text.ToString
                    'r1("grpno") = grid
    ...
    Sub RetrieveSales(ByVal trno As String)
        ...
        With dgvItem
            ...
            cmd = New MySqlCommand("SELECT h.*,w.*,s.*,c.* FROM salesh h LEFT JOIN warehouse w ON h.whscode=w.whscode " & _
                                   ...
                                   "WHERE h.isdeleted<>1 AND ttype='R' AND h.transno=@tno", cdb)
            ...
            cmd = New MySqlCommand("SELECT d.*,i.* FROM salesd d LEFT JOIN ivitem i ON d.itemcode=i.itemcode " & _
                                   "WHERE d.isdeleted<>1 and d.ttype='R' and d.transno=@tno ORDER BY d.pkno", cdb)
            cmd.Parameters.AddWithValue("@tno", trno)
            Using rd As MySqlDataReader = cmd.ExecuteReader
                If rd.HasRows Then
                    x = 0
