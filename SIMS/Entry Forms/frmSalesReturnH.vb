...
Public Class frmSalesReturnH

    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        frmSalesReturnD.ShowDialog()
    End Sub

    Private Sub frmSalesReturnH_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmSalesReturnH_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.Tag = 0
        'LoadReturnHeader()
        LoadSalesReturnHeaderNew()
    End Sub

    Sub LoadReturnHeader()
        'load to salesreturn header
    ...
    Sub LoadSalesReturnHeaderNew()
        Dim qry As String = Nothing
        With dgvInvoiceList
            .Rows.Clear()
            qry = "SELECT h.*,w.*,s.*,c.* FROM salesh h LEFT JOIN warehouse w ON h.whscode=w.whscode " & _
                                  "LEFT JOIN salesman s ON h.slmncode=s.slmncode LEFT JOIN customers c " & _
                                  "ON h.custcode=c.custcode WHERE h.isdeleted<>1 and ttype='R' " & _
        ...
        '                       "h.custcode=c.custcode WHERE h.isdeleted<>1 and ttype='R' ORDER BY transno desc limit 300", cdb)
        'rd = cmd.ExecuteReader
        'While rd.Read
        '    i += 1
    ...
    Private Sub btnEdit_Click(sender As Object, e As EventArgs) Handles btnEdit.Click
        With dgvInvoiceList
            If .RowCount > 0 Then
                ...
                frmSalesReturnD.RetrieveSales(mytrans)
                frmSalesReturnD.Tag = 1
                frmSalesReturnD.tsSave.Visible = False
                frmSalesReturnD.tsUpdate.Visible = True
                frmSalesReturnD.btnUpdate.Enabled = False
                frmSalesReturnD.btnUpdatePrint.Enabled = False
                frmSalesReturnD.Disabler()
                frmSalesReturnD.ShowDialog()
            End If
        End With
    End Sub
            ...
            If .RowCount > 0 Then
                ...
                If MsgBox("Are you sure you want to delete this Return No: " & myinvoice & "?", vbQuestion + vbYesNo, "Continue?") = vbYes Then
                    ...
                    Else
                        ...
                        MsgBox("Return No: " & myinvoice & " successfully deleted", 64, "Deleted")
    ...
    Private Sub txtSearch_TextChanged(sender As Object, e As EventArgs) Handles txtSearch.TextChanged
        ...
        If cbxViewPaid.Checked = True Then
            SearchLoadSalesreturnHeaderPaid()
        Else
            SearchLoadSalesReturnHeader()
        End If
        ...
        '                       "invoiceno like '%" & txtSearch.Text & "%' ) and  h.isdeleted<>1 and ttype='R' " & _
    ...
    Sub SearchLoadSalesReturnHeaderPaid()
        'load to sales header
        Dim qry As String = Nothing
        With dgvInvoiceList
            .Rows.Clear()
            ...
            qry = "SELECT h.*,w.*,s.*,c.* FROM salesh h LEFT JOIN warehouse w ON h.whscode=w.whscode " & _
                                   ...
                                   "ON h.custcode=c.custcode WHERE (custname like '%" & txtSearch.Text & "%' or " & _
                                   "invoiceno like '%" & txtSearch.Text & "%' ) and  h.isdeleted<>1 and ttype='R' " & _
                                   "AND applied<>'0' and totalamt=applied ORDER BY transno desc limit 300"
            Using cmd = New MySqlCommand()
                cmd.Connection = cdb
                cmd.CommandText = qry
                cmd.CommandType = CommandType.Text
    ...
    Sub SearchLoadSalesReturnHeader()
        'load to sales header
        Dim qry As String = Nothing
        With dgvInvoiceList
            .Rows.Clear()
            ...
            qry = "SELECT h.*,w.*,s.*,c.* FROM salesh h LEFT JOIN warehouse w ON h.whscode=w.whscode " & _
                                   ...
                                   "ON h.custcode=c.custcode WHERE (custname like '%" & txtSearch.Text & "%' or " & _
                                   "invoiceno like '%" & txtSearch.Text & "%' ) and  h.isdeleted<>1 and ttype='R' " & _
                                   "AND applied>='0' and applied<totalamt ORDER BY transno desc limit 300"
            Using cmd = New MySqlCommand()
                cmd.Connection = cdb
                cmd.CommandText = qry
                cmd.CommandType = CommandType.Text
    ...
    Private Sub btnGo_Click(sender As Object, e As EventArgs) Handles btnGo.Click
        If cbxViewPaid.Checked = True Then
            LoadSalesReturnHeaderByDateRangePaid()
        Else
            LoadSalesReturnHeaderByDateRange()
        End If
    End Sub
    ...
    Private Sub cbxViewPaid_CheckedChanged(sender As Object, e As EventArgs) Handles cbxViewPaid.CheckedChanged
        If cbxViewPaid.Checked = True Then
            LoadSalesReturnHeaderPaid()
        Else
            LoadSalesReturnHeaderNew()
        End If
    End Sub

    Sub LoadSalesReturnHeaderPaid()
        Dim qry As String = Nothing
        With dgvInvoiceList
            .Rows.Clear()
            qry = "SELECT h.*,w.*,s.*,c.* FROM salesh h LEFT JOIN warehouse w ON h.whscode=w.whscode " & _
    ...
    Sub LoadSalesReturnHeaderByDateRangePaid()
        ...
        With dgvInvoiceList
            ...
            qry = "SELECT h.*,w.*,s.*,c.* FROM salesh h LEFT JOIN warehouse w ON h.whscode=w.whscode " & _
                                   ...
                                   "and idate <= '" & Format(sDate2.Value, "yyyy-MM-dd") & "' and h.isdeleted<>1 and ttype='R' " & _
    ...
    Sub LoadSalesReturnHeaderByDateRange()
        Dim qry As String = Nothing
        With dgvInvoiceList
            .Rows.Clear()
            qry = "SELECT h.*,w.*,s.*,c.* FROM salesh h LEFT JOIN warehouse w ON h.whscode=w.whscode " & _
                                   ...
                                   "and tdate <= '" & Format(sDate2.Value, "yyyy-MM-dd") & "'and h.isdeleted<>1 and ttype='R' " & _
